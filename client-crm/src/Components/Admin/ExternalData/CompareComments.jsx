...
const CompareComments = ({collapsed}) => {
  ...
  const fetchComments = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/external/compBazar`, {
        headers: {
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 relative w-full">
          <div className="md:hidden sm:hidden w-full flex flex-col items-center space-y-4 mb-4">
            <div className="flex justify-center w-full items-center">
              <h1 className="text-base font-bold text-gray-800 dark:text-gray-400 mb-10">
                Compare Bazar 
              </h1>
            </div>
          </div>
