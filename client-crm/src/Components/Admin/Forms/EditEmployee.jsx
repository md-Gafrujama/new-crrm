  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/api/employee/${userId}`, {
          headers: {
...
const handleUnlock = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    await axios.put(
      `${API_BASE_URL}/api/employee/${userId}`,
      { locked: false },
      { headers: { Authorization: `Bearer ${token}` } }
    );
    setIsLocked(false);
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-2xl  transition-all">
        ...
        <div>
          ...
          <div>
          <label className="block text-sm font-semibold text-gray-700 dark:text-gray-400 mb-1" htmlFor="middleName">
            Middle Name
          </label>
          <input
            id="middleName"
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 dark:text-gray-400 mb-1" htmlFor="status">
            Status
          </label>
          <input
            id="status"
