...
const StatusItem = ({ completed, title, changedBy, description, date, note, isCurrent }) => (
  <li className="relative pl-6 pb-4 border-l-2 border-green-200 dark:border-green-600 last:border-l-0 last:pb-0">
    ...
    {!completed && !isCurrent && (
      <div className="absolute w-3 h-3 bg-[#ff8633] rounded-full w-5 h-5 -left-[10px] top-0"></div>
    )}
    ...
    <div className="ml-2">
      ...
      {/* Only show "Changed by" if status was changed (completed = true) */}
      {completed && (
        <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch users to map UIDs to names
        const usersResponse = await axios.get(`${API_BASE_URL}/api/allUser`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          }
        });
        ...
        usersResponse.data.forEach(user => {
          users[user._id] = user.username || user.email;
        });
        setUserMap(users);
          ...
          {history.length === 0 ? (
            ...
            <ul className="space-y-4">
              {history.map((version, index) => {
                ...
                const changedBy = userMap[version.userId] || version.userId;
